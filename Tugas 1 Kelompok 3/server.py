...
class Server:
    ...
    def accept_connections(self, ip_address, port):
        ...
        while True:
            ...
            print(str(address[0]) + ":" + str(address[1]) + " Berhasil Terhubung")
    ...
    def clientThread(self, connection):
        ...
        room_id = connection.recv(1024).decode().replace("Bergabung ", "")

        if room_id not in self.rooms:
            connection.send("Grup Baru Telah Dibuat".encode())
        else:
            connection.send("~~Selamat Datang~~".encode())
